...
function SearchResults(props: SearchResultsProps) {
    ...
    useEffect(() => {
        ...
        if (searchQuery.query !== lastSearchQuery.current.query)
            loadedCounter.current = 0;

        getImages(searchQuery.query, loadedCounter.current)
            .then((response) => {
                ...
                if (newSearchResult.error !== "") {
                    ...
                    console.error("ENCOUNTERED ERROR IN USEEFFECT", {
                        ...searchResult,
                        results: resultTiles,
                        error: newSearchResult.error,
                    });
                ...
                for (
                    ...
                    i <
                    Math.min(
                        newSearchResult.numberOfResults,
                        newSearchResult.imageLink.length
                    );
                    i++
                ) {
                ...
                if (loadedCounter.current === 0)
                    loadedCounter.current = newSearchResult.imageLink.length;
    ...
    function loadMoreImages(numberOfImages = 20) {
        ...
        getImages(
            ...
            if (newSearchResult.error !== "") {
                ...
                lastSearchResult.current = cloneDeep(newSearchResult);
                console.error("ENCOUNTERED ERROR IN LOADMORE", {
                    ...searchResult,
                    results: resultTiles,
                    error: newSearchResult.error,
                });
    ...
    if (lastSearchQuery.current.query !== searchQuery.query)
        return <div className="loader"></div>;

    console.log(
        searchResult.numberOfResults,
        searchResult.imageLink.length,
        searchResult.error === "",
        newSubmitEvent,
        clickBackEvent
    );
    ...
    return (
        <div id="SearchResults">
            <h3 id="top-matches">
                {numberOfResults
                    ? `${numberOfResults} matches for "${searchQuery.query.toString()}":`
                    : null}
            </h3>
