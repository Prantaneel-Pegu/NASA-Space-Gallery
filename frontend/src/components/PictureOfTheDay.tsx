...
function PictureOfTheDay () {
    ...
    getPotd()
        .then(response => {
            ...
            const errorKey = Object.keys(errorCodes).find(key => errorCodes[key as keyof typeof errorCodes] === responseClone.error);
            if (errorKey) responseClone.error = errorTexts[errorKey as keyof typeof errorTexts];
            
            if (!isEqual(responseClone, potdResults)) setPotdResults(responseClone); 
            setLoaded(true);
        })
        .catch((error: Error) => { 
            errorTexts.miscError = `Unexpected error occured of type '${error.name}'.`;
            
            if (error instanceof Error && error.message === errorCodes.netError) {
                if (potdResults.error !== errorTexts.netError) {
                    setPotdResults({...getPotdPrototype, error: errorTexts.netError});
                }
                console.warn(`Encountered a network error. \nError message: ${error.message || 'none'}`);

            } else if (error instanceof Error && error.message === errorCodes.reqLimitExceeded) {
                if (potdResults.error !== errorTexts.reqLimitExceeded) {
                    setPotdResults({...getPotdPrototype, error: errorTexts.reqLimitExceeded});
                }
                console.warn(`Too many requests. \nError message: ${error.message || 'none'}`);

            } else if (error instanceof Error && error.message === errorCodes.generalError) {
                if (potdResults.error !== errorTexts.generalError) {
                    setPotdResults({...getPotdPrototype, error: errorTexts.generalError});
                }
                console.warn(`Encountered an Error. \nError message: ${error.message || 'none'}`);

            } else {
                if (potdResults.error !== error.message && potdResults.error !== errorTexts.miscError && !Object.values(errorTexts).includes(potdResults.error)) {
                    setPotdResults({...getPotdPrototype, error: error.message || errorTexts.miscError});
                }
            ...
            <div id="Potd">
                ...
                <div id="potd-main">
                    <figure id="potd-figure">
                        <img id="potd-img" src={potdResults.url} alt={potdResults.title} />
                    ...
                    <div id="potd-data">
                        ...
                        <p id="potd-description">
                            {potdResults.description}
                        </p>
                        <code id="potd-img-address">
                            <p id="potd-address-heading">
                                Image address:
                            </p>
