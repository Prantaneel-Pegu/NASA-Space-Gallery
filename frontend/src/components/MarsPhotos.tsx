...
function MarsPhotos() {
    ...
    const [rawServerResponse, setRawServerResponse] = useState(
        cloneDeep(getRoverPhotosPrototype)
    );
    ...
    function handleSubmit(e: FormEvent<HTMLFormElement>) {
        ...
        if (!isEqual(photosRequest, newPhotosRequest))
            setPhotosRequest(newPhotosRequest);

        setRequestPending(true);
        setLoaded(false);
        setRawServerResponse(cloneDeep(getRoverPhotosPrototype));
        getRoverPhotos(newPhotosRequest, true).then((photos) => {
            ...
            for (let i = 0; i < Math.min(photos.photoUrls.length, 20); i++) {
                ...
                photoTiles.push(
                    <article className="mp-photo-tile" key={id}>
                        <img
                            src={url}
                            alt={`A photo captured by the Curiosity rover. Photo id: ${id}`}
                            className="mp-photo-tile-img"
                        />
                    </article>
                );
            }
    ...
    function loadMorePhotos(numberOfPhotos = 20) {
        ...
        getRoverPhotos(photosRequest, false).then((photos) => {
            setRawServerResponse(photos);
            ...
            for (
                ...
                i <
                Math.min(
                    numberOfPhotos + loadedCounter.current,
    ...
    console.log(
        ...
        rawServerResponse.photoUrls.length !== photosResponse.length,
        photosResponse
    );

    return (
        <div id="MarsPhotos">
            <h1 id="mp-heading">Mars Rover Photos</h1>
            <p id="mp-desc">
                See some of the most awe-inspiring pictures taken by NASA's
                Curiosity rover - operational since 9/7/2012.
                <br />
                <br />
                Total photos taken: {totalRoverPhotos || "~702642"}
            </p>
            ...
            {requestPending && !loaded ? (
                ...
                <div id="mp-results">
                    ...
                    <InfiniteScroll
                        next={loadMorePhotos}
                        hasMore={
                            rawServerResponse.photoUrls.length !==
