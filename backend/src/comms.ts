...
async function getStaticMap(latitude: string, longitude: string, width: string, height: string, zoom: string, isClient: boolean) {
    ...
    try {
        serverResponse = (await axios.get(signUrl(reqUrl, MAP_URL_SIGNING_SECRET), { responseType: "arraybuffer", signal: AbortSignal.timeout(requestTimeout) })).data;
    } 
    catch (err) {
        console.error("\x1b[31m", `[server-comms]`, "\x1b[0m", ": Encountered Error: \n", err);
    }
...
export { getImages, getImage, getKeywordsMatches, getPotd, getRoverPhotos, getStaticMap };