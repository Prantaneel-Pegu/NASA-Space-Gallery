...
import { getImages, getImage, getKeywordsMatches, getPotd, getRoverPhotos, getStaticMap } from './comms.js';
...
app.get('/api/images/search', (req, res) => {
	...
	try {
		...
		if (typeof query === 'string' && typeof page_size === 'string') {
			getImages(query, page_size)
				.then(serverResponse => {
					res.json(serverResponse);
					res.status(200).end();
				});
		...
		if (typeof keywords === 'string' && typeof page_size === 'string') {
			getKeywordsMatches(keywords, page_size)
				.then(serverResponse => {
					res.json(serverResponse);
					res.status(200).end();
				});
...
app.get('/api/staticmap', (req, res) => {
	...
	try {
		if (typeof latitude === 'string' && typeof longitude === 'string' && typeof width === 'string' && typeof height === 'string' && typeof zoom === 'string') {
			getStaticMap(latitude, longitude, width, height, zoom, isClient)
				.then(serverResponse => {
