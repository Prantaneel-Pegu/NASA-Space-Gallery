...
router.get("/api/images/search", (req, res) => {
    ...
    try {
        if (typeof imageId === "string") {
            (0, comms_js_1.getImage)(imageId).then((serverResponse) => {
                res.json(serverResponse);
                res.status(200).end();
            });
        }
        else if (typeof query === "string" && typeof page_size === "string") {
            (0, comms_js_1.getImages)(query, page_size).then((serverResponse) => {
                res.json(serverResponse);
                res.status(200).end();
            });
        }
            ...
            (0, comms_js_1.getKeywordsMatches)(keywords, page_size).then((serverResponse) => {
                res.json(serverResponse);
                res.status(200).end();
            });
        }
...
router.get("/api/potd", (req, res) => {
    ...
    console.log("\x1b[36m", "[server]", "\x1b[0m", ": Received request: ", reqUrl);
    try {
        (0, comms_js_1.getPotd)().then((serverResponse) => {
            res.json(serverResponse);
        });
    }
    catch (error) {
...
router.get("/api/roverphotos/search", (req, res) => {
    ...
    try {
        if (typeof date === "string") {
            (0, comms_js_1.getRoverPhotos)({ date: date }).then((serverResponse) => {
                res.json(serverResponse);
            });
        }
        else {
...
router.get("/api/staticmap", (req, res) => {
    ...
    try {
        if (typeof latitude === "string" &&
            ...
            typeof zoom === "string") {
            (0, comms_js_1.getStaticMap)(latitude, longitude, width, height, zoom, isClient).then((serverResponse) => {
...
router.get("/api", (req, res) => {
    ...
    console.log("\x1b[36m", "[server]", "\x1b[0m", ": Received request: ", reqUrl);
    res.json({
        message: "Welcome! This is the Nasa Space Gallery API address. Take a look at the API docs for info on usage.",
    });
});
